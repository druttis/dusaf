...
final class TaskExecutorImpl implements TaskExecutor {
    ...
    private <T> TaskImpl<T> newTask(final String name) {
        return new TaskImpl<>(eventBus, taskIdCounter.incrementAndGet(), name);
    }
    ...
    /**
     * eventDispatcher.addEventListener(String.class, $ -> sout($));
     * eventDispatcher.addEventListener(PropertyChangeEvent.class, e -> e.getName().equals("play"), sout(e));
     */
}
